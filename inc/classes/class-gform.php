...
class Gform {
	...
	public function gform_settings_menu($setting_tabs) {
		...
		$icon = WOOFLUTTER_BUILD_PATH . '/icons/money-business-and-finance-svgrepo-com.svg';
	...
	public function form_settings_fields() {
		...
		$args = [
			'title'							=> __('Sub Accounts', 'wooflutter'),
			'description'					=> sprintf(
		...
		foreach(['client', 'partner', 'staff'] as $for) {
			$args['fields'][] = [
				...
				'label'					=> sprintf(__('%s percentage Commission', 'wooflutter'), ucfirst(
	...
	public function settings_fields() {
		...
		$args = [
			...
			'fields'						=> [
				[
					'id' 					=> 'testMode',
					'label'					=> __('Test mode', 'wooflutter'),
					'type'					=> 'checkbox',
					'default'				=> false,
					'description'			=> __('Check if you want to enable test mode.', 'wooflutter'),
					'help'					=> '<strong>Test mode</strong>Check if you want to enable test mode.'
				],
				[
					'id' 						=> 'publickey',
					'label'					=> __( 'Public Key', 'wooflutter' ),
					'type'					=> 'text',
					'default'				=> true,
					// 'description'			=> __( 'Mark to enable flutterwave payment functionalities.', 'wooflutter' ),
				...
				[
					'id' 					=> 'secretkey',
					'label'					=> __( 'Secret Key', 'wooflutter' ),
					'type'					=> 'text',
					'default'				=> true,
					// 'description'			=> __( 'Mark to enable function of this Plugin.', 'wooflutter' ),
				...
				[
					'id' 						=> 'encryptionkey',
					'label'					=> __( 'Encryption Key', 'wooflutter' ),
					'type'					=> 'text',
					'default'				=> true,
					// 'description'			=> __( 'Mark to enable function of this Plugin.', 'wooflutter' ),
				...
				[
					'id' 					=> 'statusBtnLink',
					'name' 					=> 'statusBtnLink',
					'label'					=> __('Success page link', 'wooflutter'),
					'type'					=> 'select',
					'description'			=> __('Select a link for the payment returned/Success page.', 'wooflutter'),
					'default'				=> 'form',
					'help'					=> sprintf(__("%s Success page link %s Payment returned status page button link. Set homepage to setup Back to home like, site homepage link. Selecting form will set button link form entry screen link.", 'wooflutter'), '<strong>', '</strong>'),
					'options'				=> [
						'form'			=> __('Form page', 'wooflutter'),
						'home'			=> __('Home page', 'wooflutter'),
					]
				],
				[
					'id' 						=> 'amountZeroMsg',
					'label'					=> __( 'Amount required message', 'wooflutter' ),
					'description'		=> __( 'This message will be the default message on settings field if the calculated amount is zero.', 'wooflutter' ),
					'type'					=> 'text',
					'default'				=> 'You must calculate an amount to make pay and proceed. Currently calculated amount is zero or less then zero!'
				],
				[
					'id'					=> 'paymentSuccess',
					'label'					=> __( 'Success status', 'wooflutter' ),
					'description'			=> __( 'Give here a long success message that will be display on payment success page. With the confirmation message that the form submitted successfully.', 'wooflutter' ),
					'type'					=> 'text',
					'default_value'			=> "Congratulations! Your payment was successful and your form was submitted."
				],
				[
					'id' 						=> 'paymentFailed',
					'label'					=> __( 'Failed status', 'wooflutter' ),
					'description'		=> __( "Give here a long error message that will be display on payment failed/cancelled/denaid status. With the confirmation message that the form didn't submitted.", 'wooflutter' ),
					'type'					=> 'text',
				...
				[
					...
					'label'					=> __( 'Mail reminder subject', 'wooflutter' ),
					'description'		=> __( "Give here a long error message that will be display on payment failed/cancelled/denaid status. With the confirmation message that the form didn't submitted.", 'wooflutter' ),
					'type'					=> 'text'
				],
				[
					...
					'label'					=> __( 'Payment Reminder', 'wooflutter' ),
					'type'					=> 'text',
					'description'		=> sprintf(
						__( "Give here any html template that will be applied for payment reminder email template from Entry list screen. Following tags could be applicable on this template. %s", 'wooflutter' ),
					...
					'default_value'			=> esc_textarea(file_get_contents(apply_filters('wooflutter/path/fix/slashes', WOOFLUTTER_DIR_PATH . '/templates/email/payment-reminder.html')))
				],
		...
		$args['fields'][] = [
			...
			'label'					=> __('Default value for sub accounts commission', 'wooflutter'),
			'type'					=> 'template'
		];
		$args['fields'][] = [
			'id' 					=> 'enableReadOnly',
			'label'					=> __('Enable read only', 'wooflutter'),
			'type'					=> 'checkbox',
			'default'				=> false,
			'description'			=> false,
	...
	public function load_gravityform_addon() {
        ...
        require_once(untrailingslashit(WOOFLUTTER_DIR_PATH).'/inc/widgets/widget-flutterwave-payment.php'); // GFFlutterwavePaymentAddon::get_instance();
        \GFAddOn::register( 'WOOFLUTTER\Inc\GFFlutterwavePaymentAddon' );
		
        // require_once(untrailingslashit(WOOFLUTTER_DIR_PATH).'/inc/widgets/widget-flutterwave-simple.php'); // GFFlutterwaveSimpleAddon::get_instance();
        // \GFAddOn::register( 'WOOFLUTTER\Inc\GFFlutterwaveSimpleAddon' );
	...
	public function register_credit_card_field() {
		if (class_exists('GFForms')) {
			require_once(
				apply_filters('wooflutter/path/fix/slashes', untrailingslashit(WOOFLUTTER_DIR_PATH) . '/inc/widgets/class-gform-widget-flutterwave.php')
			);
	...
	public function gform_field_standard_fields($fields) {
		...
		$fields['credit_card'] = 'WOOFLUTTER\Inc\GF_FlutterWave_Credit_Card_Field';
		return $fields;
	}
	public function gform_add_field_buttons($field_groups) {
		foreach ($field_groups as &$group) {
			if ($group['name'] === 'advanced_fields') {
				$group['fields'][] = array(
					'class' => 'button',
					'value' => __('Credit Card', 'wooflutter'),
	...
	public function gform_submit_button($button, $form) {
		...
		$button_text = __('Pay with Flutterwave', 'wooflutter'); // Replace with your desired button text
		...
		<script type="text/javascript">
			...
			if(typeof fieldButtons !== 'undefined') {
				fieldButtons['credit_card'] = {
					title: '<?php echo esc_js(__('Credit Card', 'wooflutter')); ?>',
					onclick: function() {
	...
	public function gform_tooltips($tooltips) {
		...
		$tooltips['form_flutterwave'] = sprintf(
			__('%s Enable Flutterwave %s Check this option to enable Flutterwave Payment for this form. This will calculate prices, taxes and prevent submittion before payment.', 'wooflutter'),
			'<strong>', '</strong>'
		);
		$tooltips['form_alsocard'] = sprintf(
			__('%s Enable Card Payment %s Check this option to enable Credit card Payment through flutterwave for this form. This will apear a choice of both credit card or secure payment.', 'wooflutter'),
			'<strong>', '</strong>'
		) . sprintf(esc_html__('Available payment methods can be configured in your %s Flutterwave Dashboard%s.', 'wooflutter'), '<a href="https://dashboard.flutterwave.com/" target="_blank">', '</a>');
		
		$tooltips['form_field_flutterwave_default_mode'] = sprintf(
			__('%s Credit Card Default %s Check this option to set default payment method Credit card. User will be able to change direct checkout method as well.', 'wooflutter'),
			'<strong>', '</strong>'
		);
		$tooltips['form_field_enable_card_preview'] = sprintf(
			__('%s Credit Card Preview %s Enable to apear a preview credit card that could be iconic instruction.', 'wooflutter'),
			'<strong>', '</strong>'
		);
		
		$tooltips['form_accountidinput'] = sprintf(
			__("%s Account ID %s Give here the Account ID that will be included on splitting payment.%s", 'wooflutter'),
			'<strong>', '</strong>',
			sprintf(
				__('%sHere are the instructions.%s%s%s', 'wooflutter'),
				'<br/>', '<ul style="text-align: left;font-size: 12px;line-height: 13px;margin-top: 10px;border-top: 1px solid #d3d3d3;padding-top: 10px;">',
				sprintf(
					__('%sGo to %sFlutterwave subaccount list.%s%sClick over a subaccount and open detailed view.%s%sFollow the URL. Here should be an ID at the endpoint of the URL%s', 'wooflutter'),
		...
		$tooltips['form_accountidselect'] = sprintf(
			__("%s Select Account %s Select each account only once on each form. Else it might effect our transaction.", 'wooflutter'),
			'<strong>', '</strong>'
		);
		...
		$tooltips['form_comissionamount'] = sprintf(
			__("%s Commission %s Give here a percentage of amount of comission for sub accounts.", 'wooflutter'),
			'<strong>', '</strong>'
		);
		$tooltips['form_comissionflat'] = sprintf(
			__("%s Flat Amount %s Give here a flat amount of comission for sub accounts. If calculated total is less then this flat amount, it won't be applied.", 'wooflutter'),
			'<strong>', '</strong>'
		);
		$tooltips['form_subaccounts'] = sprintf(
			__("%s Sub Accounts %s Select sub accounts for serving comissions. Flat amount or percentage, both will be prioritize on ascending order. Make sure the total calculated value is less then or equal to %s if total amount.", 'wooflutter'),
			'<strong>', '</strong>', number_format_i18n(WOOFLUTTER_MAX_COMISSION, 2)
		);
		$tooltips['form_submittext'] = sprintf(
			__("%s Submit Button Text %s You can setup a submit button text here. E.g. Pay, Register, Submit.", 'wooflutter'),
			'<strong>', '</strong>'
		);
		$tooltips['form_statusBtnLink'] = sprintf(
			__("%s Success page link %s Payment returned status page button link. Set homepage to setup Back to home like, site homepage link. Selecting form will set button link form entry screen link.", 'wooflutter'),
			'<strong>', '</strong>'
		);
		$tooltips['form_fluttercard_message'] = sprintf(
			__("%s Card message %s Secure card payment message around 300 characters.", 'wooflutter'),
			'<strong>', '</strong>'
		);
		$tooltips['form_require_amount_message'] = sprintf(
			__('%s Required Amount %s Give here a message that will show if form doesn\'t provide an amount to pay.', 'wooflutter'),
			'<strong>', '</strong>'
		);
		return $tooltips;
	}
	public function gform_form_settings_fields($fields, $form) {
		$fields[$this->id] = [
			'title'  => esc_html__( 'Flutterwave Payment', 'wooflutter' ),
			'fields' => [
				[
					'name'    => 'enableFlutterwave',
					'type'    => 'toggle',
					'label'   => esc_html__( 'Enable flutterwave Payment', 'wooflutter' ),
					'tooltip' => gform_tooltip( 'form_flutterwave', '', true )
				],
				[
					'name'    => 'enableCard',
					'type'    => 'toggle',
					'label'   => esc_html__( 'Enable Card Payment', 'wooflutter' ),
					'tooltip' => gform_tooltip( 'form_alsocard', '', true ),
					'description' => __('Enable credit card payment as well.', 'wooflutter'),
					'dependency' => [
						'live'   => true,
						'fields' => [
				...
				[
					'name'    => 'comissionType',
					'type'    => 'radio',
					'label'   => esc_html__( 'Comission type', 'wooflutter' ),
					'tooltip' => gform_tooltip( 'form_comissiontype', '', true ),
					'choices' => [
						['label' => esc_html__( 'Percentage', 'wooflutter' ), 'value' => 'percentage'],
						['label' => esc_html__( 'Flat amount', 'wooflutter' ), 'value' => 'flatamount'],
					],
					'dependency' => [
						'live'   => true,
						'fields' => [
				...
				[
					...
					'type'    => 'text',
					'label'   => esc_html__( 'Commission', 'wooflutter' ),
				...
				[
					'name'    => 'flatrateAmount',
					'type'    => 'text',
					'label'   => esc_html__( 'Flat amount', 'wooflutter' ),
				...
				[
					'name'    => 'subAccounts',
					'type'    => 'checkbox',
					'label'   => esc_html__( 'Sub Accounts', 'wooflutter' ),
				...
				[
					'name'    => 'submitBtnText',
					'type'    => 'text',
					'label'   => esc_html__( 'Submit text', 'wooflutter' ),
		...
		$fields[$this->id]['fields'][] = [
			...
			'type'       => 'textarea',
			'label'      => esc_html__( 'Required Amount', 'wooflutter' ),
	...
	public function gform_form_settings_menu($setting_tabs, $form_id) {
		...
		$setting_tabs['60'] = [
			'name'         => 'flutterwave',
			'label'        => __( 'Flutterwave', 'wooflutter' ),
			'icon'         => $icon,
			'query'        => ['nid' => null],
	...
	public function gform_form_settings_page_flutterwave($subview) {
		// \GFFormSettings::page_header( __( 'Flutterwave Gateway', 'wooflutter' ) );
	...
	public function gform_entry_created($entry, $form) {
		...
		if($link && !empty($link)) {
			...
			define('WOOFLUTTER_REDIRECT_URL', $link);
		}
	}
	public function createPayLinkandGo($entry, $form, $go = true) {
		...
		if($payment_amount <= 50) {wp_die(__('You must calculate at least 50', 'wooflutter').' '.(isset($entry['currency'])?$entry['currency']:'NGN'));}
		...
		try {
			...
			$getAllSubAccounts = $WooFlutter_Flutterwave->getAllSubAccounts();
		...
		if($paymentField && isset($paymentField['enableCardPaymentMethod']) && $paymentField['enableCardPaymentMethod'] && isset($formDate['flutterwave_credit_card']) && isset($formDate['flutterwave_credit_card']['mode']) && $formDate['flutterwave_credit_card']['mode'] == 'credit') {
			...
			if(count($args['subaccounts']) <= 0) {unset($args['subaccounts']);} else {
				if(!$this->preventSubAccountComission($args, $form)) {
					wp_die(__('Payment comission splitting is not properly initiated. Please contact with the administrative.', 'wooflutter'));
				}
			}
		...
		if($link) {
			...
			if($go) {
				wp_die(
					__('Can\'t create payment link. Please contact with administrative.', 'wooflutter'),
					__('Error happening on creating payment.', 'wooflutter')
				);exit;
			} else {
				return false;
			}
	...
	public function preventSubAccountComission($args, $form) {
		...
		return ($theTotalComission <= ($args['amount'] * WOOFLUTTER_MAX_COMISSION));
	}
	...
	public function updateLink() {
		...
		if($payment_link) {
			$args['message'] = __('Payment link updated successfully!', 'wooflutter');
	...
	public function paymentRefund() {
		...
		if(($refunded + $refund_amount) > $entry['payment_amount']) {
			$args['message'] = sprintf(__('You can\'t refund more then %s.', 'wooflutter'), ($entry['payment_amount'] - $refunded).' '.$entry['currency']);
			wp_send_json_error($args);
		}
		$refundable = ($refunded + $refund_amount);
		
		try {
			$RefundedPayment = $WooFlutter_Flutterwave->refund($entry['transaction_id'], $request['amount']);
			if($RefundedPayment) {
				$refunded = gform_update_meta($entry['id'], '_paymentrefunded', $refundable);
				$args['message'] = __('Payment refunded successfully!', 'wooflutter');
				$args['hooks'] = ['payment-refunded-success'];
				$args['refund'] = $RefundedPayment;
				wp_send_json_success($args);
			} else {
				$args['message'] = __('Something went wrong. Please try again later.', 'wooflutter');
				wp_send_json_error($args);
			}
		} catch (\Exception $e) {
	...
	public function sendReminder() {
		...
		$template = $this->replaceMagicWords(stripslashes(WOOFLUTTER_OPTIONS['paymentReminder']), ['entry'=>$_POST['entry'], 'form_id'=>$_POST['form_id']]);
		// $args['template'] = $template;

		if($template) {
			wp_send_json_success($args);
		} else {
			$args['message'] = __('Mail didn\'t properly sent. Entry ID or customer mail address not found!', 'wooflutter');
			wp_send_json_error($args);
		}
	}
	...
	public function replaceMagicWords($str, $args) {
		...
		// print_r($form);
		$replaceingData = [
			'mailImagePath' => WOOFLUTTER_BUILD_IMG_URI.'/images',
		...
		if(isset($formDate['email']) && !empty($formDate['email'])) {
			wp_mail($formDate['email'], stripslashes(WOOFLUTTER_OPTIONS['paymentReminderSubject']), $str, [
	...
	public function gform_editor_js_set_default_values() {
		...
		case 'flutterwave_credit_card':
			if (!field.label)
				...
				field.label = <?php echo json_encode(esc_html__('Payment', 'wooflutter')); ?>;
				var inputs = field.inputs || [];
	
				ccNumber = new Input(field.id + ".1", <?php echo json_encode(gf_apply_filters(array('gform_card_number', rgget('id')), esc_html__('Card Number', 'wooflutter'), rgget('id'))); ?>);
				ccExpirationMonth = new Input(field.id + ".month", <?php echo json_encode(gf_apply_filters(array('gform_card_expiration', rgget('id')), esc_html__('Expiration Month', 'wooflutter'), rgget('id'))); ?>);
				ccExpirationMonth.defaultLabel = <?php echo json_encode(esc_html__('Expiration Date', 'wooflutter')); ?>;
				ccExpirationYear = new Input(field.id + ".year", <?php echo json_encode(gf_apply_filters(array('gform_card_expiration', rgget('id')), esc_html__('Expiration Year', 'wooflutter'), rgget('id'))); ?>);
				ccSecruityCode = new Input(field.id + ".3", <?php echo json_encode(gf_apply_filters(array('gform_card_security_code', rgget('id')), esc_html__('Security Code', 'wooflutter'), rgget('id'))); ?>);
				ccCardType = new Input(field.id + ".4", <?php echo json_encode(gf_apply_filters(array('gform_card_type', rgget('id')), __( 'Card Type', 'wooflutter'), rgget('id'))); ?>);
				ccName = new Input(field.id + ".5", <?php echo json_encode(gf_apply_filters(array('gform_card_name', rgget('id')), esc_html__('Cardholder Name', 'wooflutter'), rgget('id'))); ?>);
				
				enableCardPaymentMethod = new Input(field.id + ".enableCardPaymentMethod", <?php echo json_encode(gf_apply_filters(array('gform_enable_card_payment_method', rgget('id')), esc_html__('Enable multiple payment methods', 'wooflutter'), rgget('id'))); ?>);
				flutterwaveDefaultModeCard = new Input(field.id + ".flutterwaveDefaultModeCard", <?php echo json_encode(gf_apply_filters(array('gform_flutterwave_default_mode_card', rgget('id')), esc_html__('Enable multiple payment methods', 'wooflutter'), rgget('id'))); ?>);
	
				enablePreviewField = new Input(field.id + ".enablePreviewField", <?php echo json_encode(gf_apply_filters(array('gform_enable_preview_field', rgget('id')), esc_html__('Show preview card', 'wooflutter'), rgget('id'))); ?>);
	
				submitBtnText = new Input(field.id + ".submitBtnText", <?php echo json_encode(esc_html__('Submit', 'wooflutter')); ?>);
				statusBtnLink = new Input(field.id + ".statusBtnLink", <?php echo json_encode(esc_html__('Button Link', 'wooflutter')); ?>);
				...
				<?php foreach(['client', 'partner', 'staff'] as $type): ?>
					var comissionAccount_<?php echo esc_attr($type); ?> = new Input(field.id + ".comissionAccount-<?php echo esc_attr($type); ?>", <?php echo json_encode(esc_html__('Sub account', 'wooflutter')); ?>);
					inputs.push(comissionAccount_<?php echo esc_attr($type); ?>);

					var comissionType_<?php echo esc_attr($type); ?> = new Input(field.id + ".comissionType-<?php echo esc_attr($type); ?>", <?php echo json_encode(esc_html__('Comission type', 'wooflutter')); ?>);
		...
		<li class="enable_multiple_payment_methods_setting field_setting">
			...
			if($this->is_flutterwave_enabled()) {
				...
				<div class="flutterwave_settings_advanced">
					<?php include untrailingslashit(WOOFLUTTER_DIR_PATH).'/templates/dashboard/admin/field_settings.php'; ?>
				</div>
				<?php
			} else {
				?>
	...
	public function getSubAC() {
		...
		if(!$form) {$json['message'] = __('Something suspecius happens. Please try on live payment.', 'wooflutter');wp_send_json_error($json);}
		try {
			...
			if(count($subaccounts)<=0) {
				$json['message'] = __('Subaccount not found', 'wooflutter');wp_send_json_error($json);
			}
			$json['hooks'] = ['card_subac_recieved'];
			$json['message'] = __('Subaccount information recieved.', 'wooflutter');
	...
	public function cardToken() {
		...
		try {
			...
			$issuedCard = $WooFlutter_Flutterwave->processCardPayment($args);
			...
			$json['message'] = __('Successfully Issued your card. Please input the OPT just sent to your number.', 'wooflutter');
	...
	public function cardOTP() {
		...
		try {
			...
			$issuedCard = $WooFlutter_Flutterwave->processCardVerify($args);
	...
	public function gform_pre_send_email($email, $message_format, $notification, $entry) {
		...
		if($payment_status !== 'success' && !defined('WOOFLUTTER_PAYMENT_DONE')) {
