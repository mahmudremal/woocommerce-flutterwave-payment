...
$backtoLink = site_url();$backtoText = __('Back to home', 'wooflutter');
...
if($entry && !is_wp_error($entry) && isset($entry['source_url']) && !empty($entry['source_url']) && $form && !is_wp_error($form)) {
    foreach($form['fields'] as $i => $field) {
        if($field->type == 'flutterwave_credit_card' && $field->statusBtnLink && $field->statusBtnLink == 'form') {
            $backtoLink = $entry['source_url'];$backtoText = __('Back to form', 'wooflutter');
        }
    }
}
...
if(in_array($payment_status, ['success', 'successful'])) {
    if($verify) {
        ...
        wp_die(
            sprintf(
                __('We can\'t verify this transaction. Please contact with support. Transaction no is. %s', 'wooflutter'),
                '<b>'.$transaction_id.'</b>'
            ), __('Verification failed!', 'wooflutter')
        );
    }
}
...
<?php if($payment_status): ?>
  ...
  <?php get_footer(); ?>
    <?php else:
  wp_die(__('Something suspicious detected or it would be probably Failed your payment. Please try again.', 'wooflutter'));
  ?>
<?php endif; ?>